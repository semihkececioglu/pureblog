...
async function getFeaturedPosts() {
  ...
  const posts = await Post.find({ status: "published", featured: true })
    ...
    .sort({ publishedAt: -1 })
    .lean();
  return JSON.parse(JSON.stringify(posts)) as (IPost & { category: ICategory })[];
}
  ...
  return (
    <section className="mb-8">
      ...
      <div className="flex items-center gap-1">
        ...
        <a href="/feed.xml" className="text-muted-foreground hover:text-foreground hover:bg-accent p-1.5 rounded transition-colors" aria-label="RSS">
