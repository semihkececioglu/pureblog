...
async function getData() {
  await connectDB();
  const [categories, seriesList, existingTags] = await Promise.all([
...
export default async function NewPostPage() {
  const { categories, seriesList, existingTags } = await getData();
  ...
  return (
    <div>
      ...
      <PostForm categories={categories} seriesList={seriesList} existingTags={existingTags} />
    </div>
  );
}
